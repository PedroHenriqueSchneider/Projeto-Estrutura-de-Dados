...
void initialize(Fila *f, int n){//INICIANDO A FILA
    f->primeiro = f->ultimo = NULL;
    f->quantidade = 0;
    f->qnt_total = n;//TAMANHO MAXIMO DA FILA
}
int vaziaF(Fila *f){//INDICA SE A FILA ESTA FAZIA
...
void destroyF(Fila *f){//RETIRANDO TODOS OS ELEMENTOS DA FILA
    while(!vaziaF(f)){
    retira(f);
    }
}
int insere(Fila *f, int ra){//ADICIONANDO NA FILA OS ELEMENTOS
    Aluno *n = (Aluno*)malloc(sizeof(Aluno));//ALOCANDO ESPACO NA MEMORIA
    n->ra = ra;//ITEM RECEBE O ELEMEfTO


    n->link = NULL;
    if(f->ultimo){//SE O PONTEIRO *ULTIMO TIVER QUALQUER VALOR > 0, RECEBE O ENDERECO ALOCADO POR MALLOC
        f->ultimo->link = n;
    }else{
        f->primeiro = n;
    }
...
int x = 0;    
    if(!vaziaF(f)) { // f->primeiro != NULL
        ...
        if(f->primeiro == NULL){//VERIFICANDO SE ACABOU A SEQUENCIA
            f->ultimo = NULL;
        }
    f->quantidade--;
    }
